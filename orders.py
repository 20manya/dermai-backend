...
DermAI — Orders
----------------
Handles cart, wishlist, orders, payments, addresses.
Razorpay is imported lazily to avoid pkg_resources issue on Python 3.13.
"""
...
# ── Cart ──────────────────────────────────────────────────────────────────────
@router.get("/cart")
def get_cart(token: str, db: Session = None):
...
def add_to_cart(req: AddToCartRequest, token: str, db: Session = None):
    from database import SessionLocal
    if db is None:
        db = SessionLocal()
    user    = get_user(token, db)
...
def update_cart(product_id: str, req: UpdateCartRequest, token: str, db: Session = None):
    from database import SessionLocal
    if db is None:
        db = SessionLocal()
    user = get_user(token, db)
...
def remove_from_cart(product_id: str, token: str, db: Session = None):
    from database import SessionLocal
    if db is None:
        db = SessionLocal()
    user = get_user(token, db)
...
@router.delete("/cart")
def clear_cart(token: str, db: Session = None):
    from database import SessionLocal
    if db is None:
        db = SessionLocal()
    user = get_user(token, db)
...
# ── Wishlist ──────────────────────────────────────────────────────────────────
@router.get("/wishlist")
def get_wishlist(token: str, db: Session = None):
    from database import SessionLocal
    if db is None:
        db = SessionLocal()
    user  = get_user(token, db)
...
def toggle_wishlist(product_id: str, token: str, db: Session = None):
    from database import SessionLocal
    if db is None:
        db = SessionLocal()
    user    = get_user(token, db)
...
def get_addresses(token: str, db: Session = None):
    from database import SessionLocal
    if db is None:
        db = SessionLocal()
    user = get_user(token, db)
...
def add_address(req: AddAddressRequest, token: str, db: Session = None):
    from database import SessionLocal
    if db is None:
        db = SessionLocal()
    user = get_user(token, db)
    ...
    address = Address(
        ...
        name=req.name, phone=req.phone, line1=req.line1,
        line2=req.line2, city=req.city, state=req.state,
        pincode=req.pincode, is_default=req.is_default
    )
    db.add(address)
    db.commit()
...
def place_order(req: PlaceOrderRequest, token: str, db: Session = None):
    from database import SessionLocal
    if db is None:
        db = SessionLocal()
    user = get_user(token, db)
    ...
    address = db.query(Address).filter(
        Address.id == req.address_id, Address.user_id == user.id
    ).first()
    if not address:
    ...
    order    = Order(
        id=order_id, user_id=user.id, total_amount=total,
    ...
    for cart_item in cart_items:
        db.add(OrderItem(
            order_id=order_id, product_id=cart_item.product_id,
            quantity=cart_item.quantity, price_at_time=cart_item.product.price
        ))
    razorpay_data = None
...
@router.get("/orders")
def get_orders(token: str, db: Session = None):
    from database import SessionLocal
    if db is None:
        db = SessionLocal()
    user   = get_user(token, db)
    ...
    return {"orders": [
        {
            ...
            "items": [{"name": i.product.name, "brand": i.product.brand, "emoji": i.product.emoji, "quantity": i.quantity, "price": i.price_at_time} for i in o.items]
        } for o in orders
    ]}


@router.get("/orders/{order_id}")
def get_order(order_id: str, token: str, db: Session = None):
    from database import SessionLocal
    if db is None:
        db = SessionLocal()
    user  = get_user(token, db)
    ...
    return {
        ...
        "items": [{"name": i.product.name, "brand": i.product.brand, "emoji": i.product.emoji, "quantity": i.quantity, "price": i.price_at_time} for i in order.items]
    }